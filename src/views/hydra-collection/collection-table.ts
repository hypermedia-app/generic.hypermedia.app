...
function getCollectionProperties(collection: Collection) {
  const typeManagesBlock = collection.manages.find(
    managesBlock => managesBlock.property.id === rdf.type,
  )
      ...
      ${searchTemplate
        ? html`
            <url-template-form
              ...
              @submit="${search}"
              .filters="${collection.currentFilter}"
            ></url-template-form>
          `
        : ''}
              ...
              <tr>
                ${repeat(
                  ...
                  property => html`
                    <td>
                      <span
                        >${render(member[property.property.id] || '', 'leaf-value', {
